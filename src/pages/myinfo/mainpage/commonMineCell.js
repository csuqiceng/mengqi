...
function renderRightContent(props) {
  return <Text style={{color: 'gray'}}>{props.rightTitle}</Text>;
}
    ...
    <TouchableOpacity
      ...
      <View style={styles.container}>
        ...
        <View style={styles.leftViewStyle}>
          <Image
            source={props.leftIconName}
            style={{...styles.leftImgStyle, width: props.leftIconName ? 24 : 0}}
          />
          <Text style={styles.leftTitleStyle}>{props.leftTitle}</Text>
        </View>
        {/*--右边--*/}
        <View style={styles.rightViewStyle}>{rightSubView(props)}</View>
      </View>
    </TouchableOpacity>
  );
}
