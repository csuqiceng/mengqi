...
export default class ShoppingCartPage extends React.Component {
    ...
    return (
      <TouchableOpacity
        ...
        <Text style={{fontSize: 15, width: 50}}>
          {this.state.shoppingCartEdit ? '完成' : '编辑'}
        </Text>
      </TouchableOpacity>
    );
  }
  ...
  onAllChecked = () => {
    ...
    this.onChangeChecked(
      !this.state.shoppingCartAllChecked,
      this.shoppingCarts,
    );
  };
    ...
    if (this.state.cartList.length > 0) {
      return (
        <ScrollView showsVerticalScrollIndicator={false}>
          <View
            style={{flex: 1, marginLeft: 5, marginTop: 10, width: width - 10}}>
            {this.state.cartList.map((item, i) => {
              ...
              if (item.checked) {
                ...
                this.shoppingCartsPrice =
                  this.shoppingCartsPrice + item.number * item.price;
              }
              return (
                <View
                  key={i}
                  ...
                  <CheckBox
                    ...
                    onClick={() =>
                      this.onChangeChecked(!item.checked, [item.productId])
                    }
