...
export default class BaseDialog extends BaseComponent {
  ...
  show(callback, state = {}) {
    this.setState({_isShow: true, ...state}, () => {
      if (
        !this.props.showAnimationType ||
        this.props.showAnimationType == 'spring'
      ) {
  ...
  render() {
    if (
      this.state._isShow ||
      (this.props && this.props.removeSubviews === false)
    ) {
      return (
        <Animated.View
          style={{
          ...
          <TouchableOpacity
            onPress={() => {
              if (
                !this.props ||
                this.props.coverClickable ||
                this.props.coverClickable == null
              ) {
